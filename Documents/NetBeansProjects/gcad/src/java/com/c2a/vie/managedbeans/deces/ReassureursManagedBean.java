...
public class ReassureursManagedBean implements Serializable {
    ...
    public void enregistrer() {
        ...
        if(formReassureurs.getEmailreass().trim().isEmpty()){
            formReassureurs.setEmailreass("pas d'email");
        }
         if(formReassureurs.getAdressreass().trim().isEmpty()){
            formReassureurs.setAdressreass("pas d'adresse");
        }
        List<Reassureurs> list=reassureursService.reassureurall();
        if(formReassureurs.getNomreass().trim().isEmpty() || formReassureurs.getTelreass().trim().isEmpty()){
        ...
        m.addMessageInfo("modifié");
          ...
          i = list.stream().filter((list1) -> (list1.getNomreass().equals(formReassureurs.getNomreass()))).map((_item) -> 1).reduce(i, Integer::sum);
            if(i==0){
            ...
            m.addMessageInfo("enregitré");
                ...
                m.addMessageWarn("reassureur dejà enregistré");
            }
